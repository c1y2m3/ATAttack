...
class attak():
    ...
    def _thread(self,ip_address,dump):
        global semaphore
        try:
            self._creakssh(ip_address)
            ipadder, result = self._check(ip_address)
    ...
    def smb_version(self,network,dump):
        start,end = self.ip2Int(network)
        for i in range(start + 1, end):
            semaphore.acquire()
            t = threading.Thread(
                target=self._thread, args=(socket.inet_ntoa(struct.pack('>I', i)),dump))
            t.start()
